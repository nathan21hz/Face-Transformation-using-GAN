...
from encode_seg import get_seg_list
...
def encode_aio(encode_video_dir):
    ...
    for seg_length in GAP:
        a.set_encode_seg(curren_frame,seg_length)
        a.prepare_for_gen()
        for frame_index_seg in range(seg_length):
            start_time = time.time()
            frame_g = a.generate_frame(frame_index_seg)
...
GAP = get_seg_list(driving_video_dir,20)
