...
def generate_aio(source_image_dir,driving_video_dir):
    source_image_filename = get_filename(source_image_dir)
    driving_video_filename = get_filename(driving_video_dir)

    a = first_order()
    a.load_source_image(source_image_dir)
    a.load_drive_video(driving_video_dir)
    a.prepare_for_gen()
    ...
    csvfile = open('./data/{}/{}_{}.csv'.format(FILENAME,source_image_filename, driving_video_filename), 'w', newline='')
    ...
    while(curren_frame<a.length):
        ...
        if success:
            ...
            curren_frame += 1
            frame_count += 1
            print("F/R:{:.3} PSNR:{:.3} SSIM:{:.3} AKD:{:.3}".format(1/process_time,psnr,ssim,akd))
    array2video(video_output, 30,"{}_{}.mp4".format(source_image_filename, driving_video_filename))
    csvfile.close()
    print("{} {} finished".format(source_image_filename,driving_video_filename))
...
folder = os.path.exists("./data/{}".format(FILENAME))
if not folder:
    os.makedirs("./data/{}".format(FILENAME))

for i in range(0,1000,15):
    source_image_dir = ".\\res\\{}\\f{}.png".format(FILENAME,i)
    driving_video_dir = ".\\res\\{}\\{}.mp4".format(FILENAME,i)
    generate_aio(source_image_dir,driving_video_dir)
